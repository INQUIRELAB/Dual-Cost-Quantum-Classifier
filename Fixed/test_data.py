...
from trace_minimization import trace_dis
...
def _claim(theta, alpha, weight, x, reprs, entanglement, chi):
    ...
    if chi == 'trace_chi':
        y_ = _claim_trace(theta, alpha, x, reprs, entanglement)
...
def _claim_trace(theta, alpha, x, reprs, entanglement):
    """
    This function is inside _claim for fidelity_chi
    INPUT: 
        -theta: initial point for the theta parameters. The shape must be correct (qubits, layers, 3)
    ...
    C = circuit(theta_aux, entanglement)
    #for r1 in reprs:
    #   Trace=trace_dis(r1, C.r)
    Trace = [trace_dis(r1, C.r) for r1 in reprs]
    #print('td=',Trace)
    #print('reprs[y]=',r1)
    #print('C.r=',C.r)
    #print('min=',np.argmin(Trace))
