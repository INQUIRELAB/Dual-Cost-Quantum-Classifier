...
def write_summary(chi, problem, qubits, entanglement, layers, method, name,
          theta, alpha, weights, chi_value, acc_train, acc_test, epochs):
    """
...
def write_summary_acc(chi, problem,layers, method, name, acc_test):

    foldname_acc = name_folder_acc(chi, layers)
    create_folder(foldname_acc)
    file_text_acc = open(foldname_acc + '/' + name + '_accuracy.txt','a+')
    file_text_acc.write('\nNEW')
    file_text_acc.write('\nProblem = ' + problem + '\n')
...
def read_summary(chi, problem, qubits, entanglement, layers, method, name):
    ...
    if chi in ['fidelity', 'weighted_fidelity', 'trace', 'weighted_trace']: chi += '_chi'
    if chi not in ['fidelity_chi', 'weighted_fidelity_chi','trace_chi', 'weighted_trace_chi']:
        raise ValueError('Figure of merit is not valid')
    if chi == 'fidelity_chi':
        foldname = name_folder(chi, problem, qubits, entanglement, layers, method)
        if problem in ['circle', 'line', '2 lines', '6squares', '3 circles', 'wavy circles', 'wavy lines', 'non convex','crown','tricrown','squares']:
...
#==============================================================================    
    if chi == 'weighted_fidelity_chi':
        ...
        if problem in ['6squares']:
            weight = np.loadtxt(foldname + '/' + name + '_weight.txt').reshape((6, qubits))
...
def write_epochs_file_acc(chi, layers, name):
    ...
    foldname_acc = name_folder_acc(chi, layers)
    create_folder(foldname_acc)
    filename_acc = foldname_acc + '/' + name + '_epochs.txt'
    file_text_acc = open(filename_acc,'a+')
    return file_text_acc    
#Sara
#==============================================================================      
...
def write_epoch(problem, file_text, epoch, theta, alpha, chi_value, acc_train, acc_test):
    """
...
#==============================================================================
        ...
        ax.plot(s, s, 'k-')
#==============================================================================
    elif problem == '6squares':
        freq = settings
        s = np.linspace(-1,1,10)
        ax.plot(s, np.zeros(10), 'k-')
        a=np.array([-0.33,-0.33,-0.33,-0.33,-0.33,-0.33,-0.33,-0.33,-0.33,-0.33])
        b=np.array([0.33,0.33,0.33,0.33,0.33,0.33,0.33,0.33,0.33,0.33])
        ax.plot(a, s, 'k-')
        ax.plot(b, s, 'k-')
#==============================================================================
...
def name_folder_acc(chi, layers):
    """
    This function takes information from the SGD_step_by_step function and saves the accuracies for training and test sets. It is required for studying the overlearning
    INPUT: 
        -chi: cost function, to choose between 'fidelity_chi' or 'weighted_fidelity_chi'
