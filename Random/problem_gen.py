...
def problem_generator(problem, qubits, layers, chi, qubits_lab=1):
    ...
    """
    chi = chi.lower()
    if chi in ['fidelity', 'weighted_fidelity','trace']: chi += '_chi'
    if chi not in ['fidelity_chi', 'weighted_fidelity_chi','trace_chi']:
...
#==============================================================================
        ...
        theta, alpha, reprs = _line(qubits, layers, qubits_lab, chi)   
#==============================================================================
    else:
...
def _squares(qubits, layers, qubits_lab, chi):
    ...
    return theta, alpha, reprs        
#==============================================================================
def _line(qubits, layers, qubits_lab, chi):
    classes = 2
    if chi == 'trace_chi':
        reprs = representatives_tr(classes, qubits_lab)
    else:
...
def representatives_tr(classes, qubits_lab):
    """
    ...
    reprs = np.zeros((classes, 3), dtype = 'complex')
    if qubits_lab == 1:
        if classes == 0:
        ...
        if classes == 2:
            ...
            reprs[0] = np.array([0.2938926261462367, -0.5090369604551273, 0.8090169943749473])
            reprs[1] = np.array([-0.2938926261462367, 0.5090369604551273, -0.8090169943749473])
        if classes == 3:
            reprs[0] = np.array([1, 0])
        ...
        if classes == 6:
            reprs[0] = np.array([0.2938926261462367, -0.5090369604551273, 0.8090169943749473])
            reprs[1] = np.array([-0.2938926261462367, 0.5090369604551273, -0.8090169943749473])
            reprs[2] = np.array([-0.7006292692220369, -0.4045084971874737, 0.5877852522924729])
            reprs[3] = np.array([0.7006292692220369, 0.4045084971874737, -0.5877852522924729])
            reprs[4] = np.array([0.4045084971874736, -0.7006292692220369, 0.5877852522924729])
            reprs[5] = np.array([0.7006292692220369, 0.4045084971874737, -0.5877852522924729])
    ...
    if qubits_lab == 2:
        ...
        if classes == 1:
            raise ValueError('Nonsense classifier')
        if classes == 2:
            reprs[0] = np.array([0.29, -0.5, 0.8])
            reprs[1] = np.array([-0.29, 0.5, -0.8])
        if classes == 3:
