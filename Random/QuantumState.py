...
class QCircuit(object):
    ...
    def U3(self, i, theta3):
        ...
        theta_f=np.arccos(np.abs(self.psi[S])**2 - np.abs(self.psi[S_])**2) - np.pi/2
        phi_f=np.angle(self.psi[S_] / self.psi[S]) 
        self.r=np.array([np.sin(theta_f)*np.cos(phi_f),np.sin(phi_f)*np.sin(theta_f),np.cos(theta_f)])
